...
export function contextForAgent(agent) {
  const bank = loadBank();
  return { agent, owner: bank.owner, factCount: bank.facts.length, eventCount: bank.events.length, workCount: bank.works.length, onedrive: bank.onedrive, drive: bank.drive, facts: bank.facts.slice(-20).map((f) => "- " + f.text).join("\n"), recent: bank.events.slice(-8).map((e) => e.agent + ":" + e.kind + " " + e.text).join("\n") };
}
...
export function exportBank() {
  ...
  const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = DRIVE_FILE; a.click();
